...
function upgrade3_convertavatars()
{
	...
	else
	{
		...
		$contents .= "<p>Done</p><p>All avatars have been moved to the file system. The next step is performing the necessary database modifications for AllStock Gold.</p>";
	}
	$output->print_contents($contents);
	$output->print_footer($nextact);
}
...
function upgrade3_dbchanges2()
{
	...
	$db->write_query("INSERT INTO ".TABLE_PREFIX."settinggroups (gid, name, description, disporder, isdefault) VALUES (11, 'Private Messaging', 'Various options with relation to the AllStock Private Messaging system (private.php) can be managed and set here.', 11, 'yes');");
	...
	$db->write_query("INSERT INTO ".TABLE_PREFIX."settings (sid, name, title, description, optionscode, value, disporder, gid) VALUES (NULL, 'bblanguage', 'Default Language', 'The default language that AllStock should use for guests and for users without a selected language in their user control panel.', 'language', 'english', 7, 1);");
	...
	$db->write_query("INSERT INTO ".TABLE_PREFIX."settings (sid, name, title, description, optionscode, value, disporder, gid) VALUES (NULL, 'showvernum', 'Show Version Numbers', 'Allows you to turn off the public display of version numbers in AllStock.', 'onoff', 'off', 1, 1);");
