...
/**
 * AllStock 1.6
 * Copyright 2010 AllStock Group, All Rights Reserved
 *
...
// Define Custom AllStock error handler constants with a value not used by php's error handler.
define("MYBB_SQL", 20);
...
class errorHandler {
	...
	public $error_types = array( 
		...
		MYBB_SQL 						=> 'AllStock SQL Error', 
		MYBB_TEMPLATE					=> 'AllStock Template Error',
		MYBB_GENERAL		 			=> 'AllStock Error',
		MYBB_NOT_INSTALLED				=> 'AllStock Error',
		MYBB_NOT_UPGRADED				=> 'AllStock Error',
		MYBB_INSTALL_DIR_EXISTS			=> 'AllStock Error',
		MYBB_SQL_LOAD_ERROR				=> 'AllStock Error',
		MYBB_CACHE_NO_WRITE				=> 'AllStock Error',
		MYBB_CACHEHANDLER_LOAD_ERROR	=> 'AllStock Error',
	);
	
	/**
	 * Array of AllStock error types
	 *
	 * @var array
	 */
	...
	/**
	 * Is AllStock in an errornous state? (Have we received an error?)
	 *
	 * @var boolean
	 */
	...
	function error($type, $message, $file=null, $line=0)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					echo "<div class=\"php_warning\">AllStock Internal: One or more warnings occured. Please contact your administrator for assistance.</div>"; 
				}
				else
				{
					global $templates;
	...
	function email_error($type, $message, $file, $line)
	{
		...
		$message = "Your copy of AllStock running on {$mybb->settings['bbname']} ({$mybb->settings['bburl']}) has experienced an error. Details of the error include:\n---\nType: $type\nFile: $file (Line no. $line)\nMessage\n$message";

		@my_mail($mybb->settings['adminemail'], "AllStock error on {$mybb->settings['bbname']}", $message, $mybb->settings['adminemail']);
	}
	...
	function output_error($type, $message, $file, $line)
	{
		...
		{
			$mybb->settings['bbname'] = "AllStock";
		}

		if($type == MYBB_SQL)
		{		
			$title = "AllStock SQL Error";
			$error_message = "<p>AllStock has experienced an internal SQL error and cannot continue.</p>";
		...
		{
			$title = "AllStock Internal Error";
			$error_message = "<p>AllStock has experienced an internal error and cannot continue.</p>";
			if($mybb->settings['errortypemedium'] == "both" || $mybb->settings['errortypemedium'] == "error" || defined("IN_INSTALL") || defined("IN_UPGRADE"))
			{
				$error_message .= "<dl>\n";
...
<body>
	<div id="container">
		<div id="logo">
			<h1><a href="http://mybb.com/" title="MyBulletinBoard"><span class="invisible">AllStock</span></a></h1>
		</div>
		...
		<div id="content">
			...
			<div id="error">
				{$error_message}
				<p id="footer">Please contact the <a href="http://mybb.com">AllStock Group</a> for support.</p>
			</div>
		</div>
	</div>
</body>
...
EOF;
	...
	<div id="mybb_error_content">
		...
		{$error_message}
			<p id="mybb_error_footer">Please contact the <a href="http://mybb.com">AllStock Group</a> for support.</p>
		</div>
	</div>
EOF;
		}
