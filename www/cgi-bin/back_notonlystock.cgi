...
target=/var/www/html/notonlystock.html

cp $parts/part1_fz.html $target

echo "<h1>Our Small Business Candidate</h1>" >> $target
echo "<a href=http://107.170.209.150/cgi-bin/notonlystock.cgi>Refresh</a>>" >> $target

echo "<FORM action=http://107.170.209.150/cgi-bin/notonlystock.cgi method=post>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${notonly_table_1b}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${notonly_table_500m}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${notonly_table_100m}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${notonly_table_20m}" >> $target
echo "</tbody>" >> $target

echo "</table></div>" >> $target
#echo "<tfoot>${basic_header}</tfoot></table></div>" >> $target

echo "<br><a href=http://107.170.209.150/cgi-bin/notonlystock.cgi>Refresh</a>>" >> $target
...
#echo "</title>"
#echo '<meta http-equiv=refresh content="0; url=http://107.170.209.150/notonlystock.html">'
#echo "</head><body></body>"
#echo "</html>"
