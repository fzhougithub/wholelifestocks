...
target=/var/www/html/recent10_down.html

cp $parts/part1_10days.html $target

echo "<h1>Recent 10 Days Down List</h1>" >> $target
echo "<a href=http://107.170.209.150/cgi-bin/recent10_down.cgi>Refresh</a>>" >> $target
...
echo "<tbody>">> $target
mysql --login-path=root_local -s -e "${recent10_down_1b}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql --login-path=root_local -s -e "${recent10_down_500M}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql --login-path=root_local -s -e "${recent10_down_100M}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql --login-path=root_local -s -e "${recent10_down_20M}" >> $target
echo "</tbody>" >> $target

echo "</table></div>" >> $target
#echo "<tfoot>${basic_header}</tfoot></table></div>" >> $target

echo "<br><a href=http://107.170.209.150/cgi-bin/recent10_down.cgi>Refresh</a>>" >> $target
...
echo "</title>"
echo '<meta http-equiv=refresh content="0; url=http://107.170.209.150/recent10_down.html">'
echo "</head><body></body>"
echo "</html>"
