...
# $new_ticker = $in{"new_ticker"};             # Get the user's name and assign to variable
 ...
 $title=~ s/[<>?;:!,.='"]//g;
 print "<h4>Chinese Research Docs</h4>";
 print "<FORM action=http://107.170.209.150/cgi-bin/doc_china.cgi method=post>";
 print "<P>";
 ...
 if ( $title ne "" and $url ne "" and $action eq "ADD"){
   `/var/www/scripts/get_doc_china.sh "CN" "$title" "$url"`   
 }

 if ( $id ne "" and $action eq "DELETE") {
   $sth=$dbh->prepare("delete from s.docs where id=".$id);
   $sth->execute
   or die "SQL Error: $DBI::errstr\n";
 }

 $sth=$dbh->prepare($show_docs." where lang='CN' order by createddate desc");
