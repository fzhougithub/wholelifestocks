...
target=/var/www/html/daily_down_v.html

cp $parts/part1_down_v.html $target

echo "<h1>Daily Down Volume Rate > 20</h1>" >> $target
echo "<a href=http://107.170.209.150/cgi-bin/refresh_down_volume.cgi>Refresh</a>>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${daily_down_v_1b}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${daily_down_v_500M}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${daily_down_v_100M}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${daily_down_v_20M}" >> $target
echo "</tbody>" >> $target

echo "<tfoot>${basic_header}</tfoot></table></div>" >> $target

echo "<br><a href=http://107.170.209.150/cgi-bin/refresh_down_volume.cgi>Refresh</a>>" >> $target
...
echo "</title>"
echo '<meta http-equiv=refresh content="0; url=http://107.170.209.150/daily_down_v.html">'
echo "</head><body></body>"
echo "</html>"
