...
target=/var/www/html/daily_up.html

cp $parts/part1_up.html $target

echo "<h1>Daily Up List</h1>" >> $target
echo "<a href=http://107.170.209.150/cgi-bin/refresh_up_sorting.cgi>Refresh</a>>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${daily_up_table_1b}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${daily_up_table_500M}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${daily_up_table_100M}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${daily_up_table_20M}" >> $target
echo "</tbody>" >> $target

echo "</table></div>" >> $target
#echo "<tfoot>${basic_header}</tfoot></table></div>" >> $target

echo "<br><a href=http://107.170.209.150/cgi-bin/refresh_up_sorting.cgi>Refresh</a>>" >> $target
...
echo "</title>"
echo '<meta http-equiv=refresh content="0; url=http://107.170.209.150/daily_up.html">'
echo "</head><body></body>"
echo "</html>"
