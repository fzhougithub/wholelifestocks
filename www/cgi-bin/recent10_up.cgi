...
target=/var/www/html/recent10_up.html

cp $parts/part1_10days.html $target

#echo ${recent10_up_1b} >> $target
#echo ${recent10_up_500M}>> $target
#echo ${recent10_up_200M}>> $target
#echo ${recent10_up_100M}>> $target

echo "<h1>Recent 10 Days Up List</h1>" >> $target
echo "<a href=http://107.170.209.150/cgi-bin/recent10_up.cgi>Refresh</a>>" >> $target
...
echo "<tbody>">> $target
mysql --login-path=root_local -s -e "${recent10_up_1b}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql --login-path=root_local -s -e "${recent10_up_500M}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql --login-path=root_local -s -e "${recent10_up_100M}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql --login-path=root_local -s -e "${recent10_up_20M}" >> $target
echo "</tbody>" >> $target

echo "</table></div>" >> $target
#echo "<tfoot>${basic_header}</tfoot></table></div>" >> $target

echo "<br><a href=http://107.170.209.150/cgi-bin/recent10_up.cgi>Refresh</a>>" >> $target
...
echo "</title>"
echo '<meta http-equiv=refresh content="0; url=http://107.170.209.150/recent10_up.html">'
echo "</head><body></body>"
echo "</html>"
