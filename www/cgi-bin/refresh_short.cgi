...
target=/var/www/html/auto_short.html

cp $parts/part1_auto_short.html $target

echo "<h1>Auto Short Candidates</h1>" >> $target
echo "<p>Be careful:Candidates here are not definitely shortable, some of them are excellent long candidates!</P>" >>$target
echo "<a href=http://107.170.209.150/cgi-bin/refresh_short.cgi>Refresh</a>>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${auto_short_1b}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${auto_short_500m}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${auto_short_100m}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${auto_short_20m}" >> $target
echo "</tbody>" >> $target

echo "</table></div>" >> $target
#echo "<tfoot>${basic_header}</tfoot></table></div>" >> $target

echo "<br><a href=http://107.170.209.150/cgi-bin/refresh_short.cgi>Refresh</a>>" >> $target
...
echo "</title>"
echo '<meta http-equiv=refresh content="0; url=http://107.170.209.150/auto_short.html">'
echo "</head><body></body>"
echo "</html>"
