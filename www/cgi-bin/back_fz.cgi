...
target=/var/www/html/daily_ours.html

/var/www/scripts/headergen.sh ourlist > $target

echo "<h1>Our new List</h1>" >> $target
echo "<a href=http://107.170.209.150/cgi-bin/ourlist.cgi>Refresh</a>>" >> $target

echo "<FORM action=http://107.170.209.150/cgi-bin/ourlist.cgi method=post>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${our_table_1b}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${our_table_500m}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${our_table_100m}" >> $target
echo "</tbody>" >> $target
...
echo "<tbody>">> $target
mysql -uroot -pondemand -s -e "${our_table_20m}" >> $target
echo "</tbody>" >> $target

echo "</table></div>" >> $target
#echo "<tfoot>${basic_header}</tfoot></table></div>" >> $target

echo "<br><a href=http://107.170.209.150/cgi-bin/ourlist.cgi>Refresh</a>>" >> $target
...
#echo "</title>"
#echo '<meta http-equiv=refresh content="0; url=http://107.170.209.150/daily_ours.html">'
#echo "</head><body></body>"
#echo "</html>"
