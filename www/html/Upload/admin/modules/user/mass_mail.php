...
if($mybb->input['action'] == "send")
{
	...
	if($mybb->input['step'] == 4)
	{
		...
		{
			$format_preview .= "{$lang->text_based} - <a href=\"#\" onclick=\"javascript:AllStock.popupWindow('index.php?module=user-mass_mail&amp;action=preview&amp;mid={$email['mid']}&amp;format=text', 'preview', 450, 450);\">{$lang->preview}</a>";
		}
		if($email['format'] == 2)
		{
		...
		{
			$format_preview.= "{$lang->html_based} - <a href=\"#\" onclick=\"javascript:AllStock.popupWindow('index.php?module=user-mass_mail&amp;action=preview&amp;mid={$email['mid']}', 'preview', 450, 450);\">{$lang->preview}</a>";
		}
